...
const Home = () => {
  ...
  return (
    <div className="container" style={{ textAlign: 'center', marginTop: '4rem' }}>
      ...
      {user ? (
        <>
          <p style={{ fontSize: '1.5rem', marginBottom: '2rem' }}>Welcome back, {user.name} ({user.role})!</p>
  ...
  return (
    <nav style={{ background: 'white', padding: '1rem', boxShadow: 'var(--shadow-sm)' }}>
      <div className="container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Link to="/" style={{ color: 'var(--primary-color)', margin: 0, fontSize: '1.5rem', fontWeight: 'bold' }}>RestoApp</Link>
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
          ...
          {user ? (
            <>
              ...
              <button onClick={handleLogout} style={{ background: 'none', border: 'none', color: 'var(--error-color)', cursor: 'pointer' }}>Logout</button>
            </>
          ) : (
            <>
