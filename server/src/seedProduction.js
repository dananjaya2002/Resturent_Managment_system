...
const seedProduction = async () => {
    try {
        ...
        console.log('Database:', process.env.MONGO_URI ? 'Using MONGO_URI from .env' : '❌ MONGO_URI not found!');
        ...
        for (const userData of demoUsers) {
            ...
            if (existingUser) {
                console.log(`  ⚠️  ${userData.role.padEnd(10)} | ${userData.email.padEnd(25)} - Already exists`);
        ...
        for (const table of tables) {
            ...
            if (!existing) {
                ...
                console.log(`  ⚠️  Table #${table.tableNumber} (Capacity: ${table.capacity}) - Already exists`);
        ...
        console.log(`  Categories:  ${stats.categories.created} created, ${stats.categories.existing} existing`);
        console.log(`  Menu Items:  ${stats.menuItems.created} created, ${stats.menuItems.existing} existing`);
        console.log(`  Tables:      ${stats.tables.created} created, ${stats.tables.existing} existing`);
        ...
        demoUsers.forEach(user => {
            console.log(`  ${user.role.toUpperCase().padEnd(10)} → ${user.email.padEnd(25)} | Password: ${user.password}`);
        });
        console.log('═════════════════════════════════════════\n');
